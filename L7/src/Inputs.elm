...
createAccView : Model -> Html Msg
createAccView model = 
    let
        check = isPasswordStrongEnough model.accountType model.password
    in
    
    div []
        [
            button [ disabled (model.username == "" || model.password == "" ||
            not check) ] [ text "Create account" ]
            , p [] [ text <| if check then "" else "Your password is not strong enough" ]
        ]


isPasswordStrongEnough : AccountType -> String -> Bool
isPasswordStrongEnough accType pass =
    case accType of
       Admin -> if (String.length pass) < 12 then False else True
       User -> if (String.length pass) < 8 then False else True

...
monthsBetween dA dB =
    case (month dA , month dB) of
       (Just m, Just m2) ->    if year dA == year dB then 
                                     Just (monthsBetweenMonths m m2)
                               else 
                                    if (year dA  < year dB) then
                                           if monthToInt m  >  monthToInt m2 then
                                                         Just ( 12 * ( year dB  - year dA) - monthsBetweenMonths m m2)
                                           else 
                                                         Just ( 12 * ( year dB  - year dA)  + monthsBetweenMonths m m2 )
                                    else
                                            if monthToInt m  >  monthToInt m2 then
                                                        Just ( 12 * (year dA  - year dB) + monthsBetweenMonths m m2)
                                             else
                                                        Just ( 12 * ( year dA  - year dB)  - monthsBetweenMonths m m2 )
...
compare (Date d1) (Date d2) =
   if d1.year < d2.year then LT
            ...
            case (d1.month, d2.month) of
                    ...
                    (Nothing, Just _) -> GT
                    (Just _, Nothing) -> LT
                    (Just m1, Just m2) -> if monthToInt m1 > monthToInt m2 then GT 
                                          else
                                          if monthToInt m1 < monthToInt m2 then LT 
                                          else EQ
...
dateToString date =
    case date of
       ...
       Just d ->
            case  month d of 
            Just m -> monthToString m ++ " " ++ String.fromInt(year d)
            Nothing -> String.fromInt(year d)
